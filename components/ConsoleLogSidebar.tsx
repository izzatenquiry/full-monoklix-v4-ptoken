...
const ConsoleLogSidebar: React.FC<ConsoleLogSidebarProps> = ({ isOpen, onClose }) => {
  ...
  const getLevelClasses = (level: LogEntry['level']) => {
    switch (level) {
      ...
      default: // 'log'
        return 'bg-transparent border-neutral-200 dark:border-white/10 text-neutral-600 dark:text-neutral-400';
    }
  };
  ...
  return (
    <>
      {/* Overlay for mobile */}
      <div
        className={`fixed inset-0 bg-black/60 backdrop-blur-sm z-30 lg:hidden transition-opacity ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
        onClick={onClose}
        aria-hidden="true"
      ></div>
      <aside
        className={`bg-white/90 dark:bg-[#121212]/80 backdrop-blur-xl border-l border-neutral-200 dark:border-white/10 flex flex-col 
                   transition-all duration-300 ease-custom-ease overflow-hidden shadow-2xl
      ...
      >
        <div className="w-full lg:w-96 h-full flex flex-col p-4">
            ...
            <div className="flex items-center gap-2">
                <button onClick={clearLogs} className="p-2 text-neutral-500 hover:text-neutral-800 dark:hover:text-white transition-colors" aria-label={T.clearLogs} title={T.clearLogs}>
                <TrashIcon className="w-5 h-5" />
                </button>
                <button onClick={onClose} className="p-2 text-neutral-500 hover:text-neutral-800 dark:hover:text-white transition-colors" aria-label={T.closeConsole}>
                ...
                {logs.length === 0 ? (
                    ...
                    logs.map((log, index) => (
                        <div key={index} className={`p-2 rounded-md border-l-4 text-xs font-mono break-words ${getLevelClasses(log.level)}`}>
                            <div className="flex justify-between items-center text-neutral-500 dark:text-neutral-500 mb-1 opacity-80">
